  ...
  return (
    <div className="bg-blue-500 w-[14%]  h-screen space-y-20 text-white  py-8 fixed">
      <div className="px-2 h-full flex flex-col text-center justify-between">
          ...
          <ul className="text-md w-full space-y-6 flex flex-col">
            {leftSidebarOptions.map((opt) => (
              <Link
                to={opt.path}
                key={opt.id}
                className={`hover:bg-black/10 p-2 rounded-3xl cursor-pointer ${currentUrlPath === opt.path && "bg-black/10"}`}
              >
                {opt.label}
              </Link>
            ))}
        ...
        <button
          onClick={logout}
          className="bg-red-500 w-full  py-1 rounded-full px-6 cursor-pointer hover:bg-red-600 transition-colors duration-300"
        >
          Logout
        </button>
      </div>
